# -*- coding: utf-8 -*-
"""
Created on Mon Jul 10 22:15:10 2023
...
from opacus.utils.batch_memory_manager import BatchMemoryManager  # memory-safe microbatching
...
class Config:
    # Data
    data_dir: str = "./physionet_sepsis"  # folder containing many .psv files
    recursive: bool = True                # search subfolders
    file_limit: Optional[int] = 10000     # cap number of patients loaded (None = no cap)
...
def list_psv_files(data_dir: str, recursive: bool = True, file_limit: Optional[int] = None) -> List[str]:
    """
    Recursively collect .psv files (handles .psv/.PSV), de-duplicate, and skip zero-byte files.
    Optionally cap count via file_limit.
    """
...
def get_variable_list_from_any(files: List[str]) -> List[str]:
    """
    Read headers from the first readable file that contains SepsisLabel.
    Returns input variable names (excludes SepsisLabel).
    """
    for fp in files:
        try:
...
def load_sepsis_dataset(
    ...
    # Try a pretty progress bar; fall back to manual % logs
    use_tqdm = False
    try:
        from tqdm.auto import tqdm  # optional, nice progress
        iterator = tqdm(files, desc="Loading patients", unit="file")
        use_tqdm = True
    except Exception:
        iterator = files
        step = max(1, n_files // 100)  # ~1% progress steps
...
# =====================================================================
def _microbatch_epoch(mem_loader, model, opt, step_fn):
    """
    Runs one epoch over a memory-managed loader with a provided step function.
    step_fn(xb) must return a scalar loss (already computed).
    """
    losses = []
    for xb in mem_loader:
        if isinstance(xb, (list, tuple)):
            xb = tuple(t.cuda() if t.is_cuda else t for t in xb)
...
def train_vae_dp(model: VAE, loader, cfg: Config):
    ...
    for e in range(cfg.vae_epochs):
        ...
        while True:
            try:
                with BatchMemoryManager(
                    ...
                    epoch_loss = _microbatch_epoch(mem_loader, model, opt, step_fn)
                break  # success
                ...
                if "out of memory" in str(err).lower() and cfg.device == "cuda":
                    ...
                    if new_phys == cfg.physical_batch_size:
                        raise
        ...
        print(f"[VAE {e+1}/{cfg.vae_epochs}] loss={epoch_loss:.6f} mae={np.mean(mae_list):.6f} | "
...
def main(cfg: Config):
    ...
    # DP Labeler (on real flat features)
    labeler, pe_lab = train_dp_labeler(X_flat, y_real, cfg)
    eps_lab = pe_lab.get_epsilon(cfg.delta)
    print(f"[Privacy Labeler] epsilon={eps_lab:.3f}, delta={cfg.delta}")
    ...
    # Optional: write first N synthetic patients to .psv for inspection
    ...
    eps_total = eps_a + eps_b + eps_lab
    print(f"[Composed DP] epsilon≈{eps_total:.3f}, delta≈{3*cfg.delta:.2e}")
