# -*- coding: utf-8 -*-
"""
Created on Mon Jul 10 04:21:35 2023
...
class Config:
    ...
    file_limit: Optional[int] = None      # cap number of patients loaded (None = no cap)
...
def load_sepsis_dataset(
    ...
    # Determine variables from any readable file
    ...
    for idx, fp in enumerate(iterator, 1):
        try:
            ...
            if use_tqdm:
                # show we skipped one, but don't spam
                pass
            else:
                if idx % step == 0 or idx == n_files:
                    print(f"[LOAD] {idx}/{n_files} ({idx / n_files:>6.2%}) ... (skipped unreadable)")
            continue

        if "SepsisLabel" not in df.columns:
            if not use_tqdm and (idx % step == 0 or idx == n_files):
                print(f"[LOAD] {idx}/{n_files} ({idx / n_files:>6.2%}) ... (missing SepsisLabel)")
            continue
