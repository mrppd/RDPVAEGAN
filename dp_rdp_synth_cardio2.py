# -*- coding: utf-8 -*-
"""
Created on Fri Jun 9 12:45:25 2023
...
def train_gan_dp(decoder: nn.Module, input_dim: int, train_loader, cfg: Config):
    decoder = decoder.to(cfg.device)   # ensure device alignment
    ...
    for e in range(cfg.gan_epochs):
        ...
        for real in priv_loader:
            ...
            z = torch.randn(bs, cfg.noise_dim, device=cfg.device)
            fake_g = decoder(G(z))        # decoder is CLEAN (no hooks) and on device
...
def invert_to_raw_df(Xsyn: np.ndarray, feat_names: list, meta: dict) -> pd.DataFrame:
    ...
    # 1) Invert z-scoring on numeric columns
    for col, (m, s) in meta.get("numeric_stats", {}).items():
        if col in df.columns:
            df[col] = df[col] * s + m

    # 2) Collapse one-hot groups -> original categorical columns
    ...
    # 3) Cast/clip categorical & integer-like fields
    ...
    # 4) Convert age back to days (dataset’s original unit)
    if "age" in df.columns:
        df["age"] = np.rint(df["age"] * 365.25).astype(int)

    # 5) Recompute engineered features from raw numerics
    for c in ["bmi", "pp"]:
        if c in df.columns:
    ...
    # Order like the original dataset (+ engineered cols at end)
...
def main(cfg: Config):
    ...
    # Save model-space (standardized + one-hot) for ML use
